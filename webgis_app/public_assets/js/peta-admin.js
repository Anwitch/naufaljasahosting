...
const ICONS = {
    ...
    rumah:   makeIcon('🕌', COLORS.rumah),  // default (Islam)
    warga:   makeIcon('👤', COLORS.warga),
    blankSpot: makeIcon('⚠️', COLORS.blankSpot, 36)
};

/* ── Per-religion icon colors & emojis ── */
const AGAMA_EMOJI = {
    'Islam':    '🕌',
    'Kristen':  '⛪',
    'Katolik':  '⛪',
...
function getRumahIcon(agama) {
    const emoji = AGAMA_EMOJI[agama] || '🛐';
    const color = AGAMA_COLOR[agama] || COLORS.rumah;
...
/* ── Render: Rumah Ibadah (Point) — icon berubah sesuai agama ── */
function renderRumah(data) {
    ...
    (data.features || []).forEach((f) => {
        ...
        const agama = props.agama || 'Islam';
        const agamaColor = AGAMA_COLOR[agama] || COLORS.rumah;
        ...
        const circle = L.circle(ll, {
            ...
            color: agamaColor,
            fillColor: agamaColor,
            fillOpacity: 0.10,
            opacity: 0.60,
            weight: 2
        });
        const marker = L.marker(ll, { icon: getRumahIcon(agama) });
...
function bindPopup(layer, type, props) {
    ...
    // Map type to the correct admin manage page
    const kelolaMap = {
        spbu:    'kelola_spbu.php',
        jalan:   'kelola_jalan.php',
        kavling: 'kelola_kavling.php',
        kawasan: 'kelola_kawasan.php',
        rumah:   'kelola_fasilitas.php',
        warga:   'kelola_warga.php'
    };
    const kelolaPage = kelolaMap[type] || '';
    html += `<div class="popup-actions">
        <button class="btn btn-danger btn-sm" onclick="deleteFeature('${type}', ${props.id})"><i class="fas fa-trash"></i> Hapus</button>
        ${kelolaPage ? `<button class="btn btn-ghost btn-sm" onclick="window.location.href=APP_BASE+'/panel_admin/${kelolaPage}'"><i class="fas fa-edit"></i> Kelola</button>` : ''}
    </div>`;
    ...
    if (type === 'spbu') {
        ...
        formHtml = `
            ...
            <div class="form-group"><label class="form-label">Agama *</label>
                <select id="f-agama" class="form-control" onchange="updateRumahModalTitle(this.value)">
...
/* ── Update draw modal title when agama changes ── */
window.updateRumahModalTitle = function(agama) {
    const emoji = AGAMA_EMOJI[agama] || '🛐';
    document.getElementById('formModalTitle').textContent = `${emoji} Tambah Rumah Ibadah`;
};
